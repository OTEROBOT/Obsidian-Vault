...
-- Project: Obsidian Vault (itdepagafihwvtklxfql)
-- Storage Bucket: vault-media
-- Admin: oterobot@gmail.com
-- =========================================================
...
-- 6. Setup RLS Policies (Allow Read for everyone; Write restricted to verified Admin: oterobot@gmail.com)
CREATE OR REPLACE FUNCTION public.is_vault_admin()
RETURNS boolean AS $$
BEGIN
  RETURN (lower(coalesce(auth.jwt() ->> 'email', '')) = 'oterobot@gmail.com');
END;
...
-- 7. Configure Supabase Storage Bucket 'vault-media'
...
FOR INSERT WITH CHECK (bucket_id = 'vault-media' AND public.is_vault_admin());
...
FOR UPDATE USING (bucket_id = 'vault-media' AND public.is_vault_admin());
...
FOR DELETE USING (bucket_id = 'vault-media' AND public.is_vault_admin());
