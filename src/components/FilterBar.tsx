...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const mediaTypes: { type: 'all' | MediaType; label: string; icon: React.ReactNode; color: string }[] = [
    { type: 'all', label: 'All Media', icon: <Layers className="w-3.5 h-3.5" />, color: 'text-slate-300' },
    { type: 'video', label: 'Video', icon: <VideoIcon className="w-3.5 h-3.5" />, color: 'text-rose-400' },
    { type: 'audio', label: 'Audio', icon: <Music className="w-3.5 h-3.5" />, color: 'text-purple-400' },
    { type: 'image', label: 'Images', icon: <ImageIcon className="w-3.5 h-3.5" />, color: 'text-amber-400' },
    { type: 'web', label: 'Web / Articles', icon: <Globe className="w-3.5 h-3.5" />, color: 'text-emerald-400' },
  ];
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-medium whitespace-nowrap transition-all duration-200 border select-none ${
                isSelected
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 transition-all"
            >
      ...
      {tags.length > 0 && (
        <div className="flex items-center gap-1.5 flex-wrap pt-1 text-xs">
          <span className="text-[11px] font-mono uppercase tracking-wider text-slate-500 flex items-center gap-1 mr-1">
            <TagIcon className="w-3 h-3 text-cyan-400" /> Tags:
          </span>
          <button
          ...
          {tags.map((t) => {
            const isTagActive = filters.tag === t.name;
            return (
              <button
                key={t.id}
                onClick={() => onFilterChange({ ...filters, tag: isTagActive ? 'all' : t.name })}
